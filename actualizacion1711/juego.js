...
cargaPantalla() {
 ...
 pantallaGanaste() {
  ...
  this.timer = 15; // Reinicia el temporizador para ganar/perder
  this.timerCreditos = 25; // Reinicia el temporizador de créditos
  for (let arbol of this.arboles) {
...
mostrarCreditos() {
  if (this.estado === 2) { // Estado de créditos o pérdida
    ...
    // Temporizador para reiniciar el juego
    if (frameCount % 60 === 0 && this.timer > 0) {
      this.timer--;
    }
    if (this.timer === 0) {
