class micelio {
  ...
  if (direccion === 's' || direccion === 'w' || direccion === 'x' || direccion === 'a' || direccion === 'q' || direccion === 'z') {
    ...
    nuevaPos.y = constrain(nuevaPos.y - 25 * sin(randomAngle), 0, height+20);
  } else if (direccion === 'r' || direccion === 't' || direccion === 'y' || direccion === 'u' || direccion === 'g' || direccion === 'f') {
    nuevaPos.y = constrain(nuevaPos.y - 25 * cos(randomAngle), 0, height+20);
...
reiniciarJuego() {
  objJuego = new Juego();
  objJuego.crearM();
  objJuego.tiempoIgnorado = 0;
  objJuego.perder = false;
}
}
